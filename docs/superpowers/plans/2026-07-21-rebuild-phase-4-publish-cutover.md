...
- **Sheet normalization policy (resolves the tracked ''-vs-NULL + postnummer notes):** at EXPORT, text NULLs render as `""` and postnummer renders as a 4-digit zero-padded string — the sheet looks identical regardless of which pipeline wrote the row.
