...
- `dnb_rows(conn)` — the FIRST WORKING DNB export: columns per the legacy DNB tab (read `scripts/sync_dnbeiendom_sheet.py` + `scripts/export_dnbeiendom_to_sheet.py` for the header/format), travel values from the new dnbeiendom columns. DNB-ONLY-UNIQUE rows (duplicate_of_finnkode NULL/blank), matching legacy's hard filter (sync_dnbeiendom_sheet.py:100-105) — PLAN DEFECT FIXED IN REVIEW: the original matched-rows mandate would double-pin the map (DNB payload has no Finnkode column, Code.gs can't dedupe it).
