...
- Intentional behavior changes vs legacy (there are exactly two sanctioned: robust finnkode parsing via `urllib.parse`, and dropping the `len(href) <= 100` URL heuristic) must appear in `config/verify-allowlist.toml` with a justification, and nowhere else.
